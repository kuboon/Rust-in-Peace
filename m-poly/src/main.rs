...
fn m2v(x:MP) -> MVX //多項式から配列型への変換
{
  ...
  for i in 0..x.t
  {
    if x.x[i as usize].a > 0 {
      z.m[x.x[i as usize].n[0] as usize][x.x[i as usize].n[1] as usize] = x.x[i as usize].a as i16;
    }
  }
...
fn printv(x:MP)
{
  ...
  for i in 0..x.t
  {
    if x.x[i as usize].a > 0
    {
      print!("{}*x^{}*y^{}+", x.x[i as usize].a, x.x[i as usize].n[0], x.x[i as usize].n[1]);
    }
  }
}
  ...
  for l in 0..100
  {
    for i in 0..100
    {
      for j in 0..100
      {
        ...
        {
            c.m[i + j][k + l] ^= GF[mlt((FG[x.m[j as usize][k as usize] as usize]) as i32 , (FG[y.m[i as usize][l as usize] as usize]) as i32) as usize] as i16;
        }
        }
      }
    }
